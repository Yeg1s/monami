...
- Сохраняй направление желания: если автор хочет что-то ДЛЯ СЕБЯ ("сделай мне", "хочу получить") — фраза про автора. Если автор хочет что-то СДЕЛАТЬ ДРУГОМУ ("хочу сделать ему/ей массаж") — фраза про получателя.
- Используй метафоры и иносказания — не пиши буквально, пусть смысл угадывается
- Чередуй стили: иногда через пикантную метафору, иногда через забавное иносказание
- Образы бери из еды, природы, сказок, путешествий — но подавай дерзко
